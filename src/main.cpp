...
unsigned int nModifierInterval = 46 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 100; // 100 Confirmations
...
const string strMessageMagic = "Leduro Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (pindexBest->nHeight < 900000)
    {
        nSubsidy = 1 * COIN;
    }
...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  // 25% Per Annual year in interest 

    return nSubsidy + nFees;
}

static int64_t nTargetTimespan = 46 * 60;  // 46 minutes for the timespan to adjust
...
{
    int64_t ret =  blockValue * 3/5; // 60% of the Block Value
    return ret;
}
