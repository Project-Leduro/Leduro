...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #ledurocoinTEST\r");
            Send(hSocket, "WHO #ledurocoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #ledurocoin\n");
            // randomly join #ledurocoin00-#ledurocoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ledurocoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ledurocoin%02d\r", channel_number).c_str());
        };
