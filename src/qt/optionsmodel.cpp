...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLeduroAmount"))
        settings.setValue("nAnonymizeLeduroAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeLeduroAmount = settings.value("nAnonymizeLeduroAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeLeduroAmount"))
        SoftSetArg("-anonymizeLeduroamount", settings.value("nAnonymizeLeduroAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeLeduroAmount:
            nAnonymizeLeduroAmount = value.toInt();
            settings.setValue("nAnonymizeLeduroAmount", nAnonymizeLeduroAmount);
            emit anonymizeLeduroAmountChanged(nAnonymizeLeduroAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
