...
{
    // Convert leduro:// to leduro:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("leduro://"))
    {
        uri.replace(0, 12, "leduro:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Leduro.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the ILeduroLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query ILeduroLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "leduro.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Leduro-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  leduro-qt [" + tr("command-line options") + "]                     " + "\n";
